...
export function usePollSocket(pollId?: string | null) {
    ...
    const setPollStatus = usePollStore((state) => state.setPollStatus)
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            socket.on('poll:published', (payload) => {
                if (payload.pollId) setPollStatus(payload.pollId, 'published')
                if (payload.analytics) setAnalytics(payload.analytics)
            })
            socket.on('poll:closed', (payload) => {
                if (payload.pollId) setPollStatus(payload.pollId, 'closed')
                if (payload.analytics) setAnalytics(payload.analytics)
            })
            socket.on('poll:reopened', (payload) => {
                if (payload.pollId) setPollStatus(payload.pollId, 'active')
                if (payload.analytics) setAnalytics(payload.analytics)
            })
    ...
    }, [pollId, setAnalytics, setPollExpired, setPollStatus])
}
