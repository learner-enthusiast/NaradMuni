...
export function DashboardPage() {
    ...
    const { activePoll, analytics, setActive, setLoading, loading } =
        usePollStore()
        ...
        async function load() {
            ...
            try {
                const [data, responseData] = await Promise.all([
                    api.get<{ poll: Poll; analytics: Analytics }>(
                        `/api/poll/${pollId}/analytics`
                    ),
                    api.get<{ responses: ResponseDetail[] }>(
                        `/api/poll/${pollId}/responses`
                    ),
                ])
                if (mounted) {
    ...
    useEffect(() => {
        ...
        let mounted = true
        api.get<{ responses: ResponseDetail[] }>(
            `/api/poll/${activePoll.id}/responses`
        )
            .then((responseData) => {
            ...
            .catch((err) => {
                if (mounted)
                    setError(getApiError(err, 'Could not refresh responses'))
            })
    ...
    const publish = async () => {
        ...
        try {
            const data = await api.post<{ poll: Poll; analytics: Analytics }>(
                `/api/poll/${activePoll.id}/publish`
            )
    ...
    const exportCsv = async () => {
        ...
        try {
            const blob = await api.download(
                `/api/poll/${activePoll.id}/export.csv`
            )
            ...
            <div className="db-layout">
                ...
                <div className="db-page-head">
                    ...
                    <div className="db-head-actions">
                        ...
                        {activePoll.status === 'closed' ? (
                            ...
                            <button
                                className={`db-btn db-btn-primary ${publishing || activePoll.status === 'published' ? 'cursor-not-allowed' : null}`}
                                disabled={
                                    statusChanging ||
                                    activePoll.status === 'published'
                                }
                        ...
                        <button
                            className={`db-btn db-btn-primary ${publishing || activePoll.status === 'published' ? 'cursor-not-allowed' : null}`}
                            disabled={
                                publishing || activePoll.status === 'published'
                            }
                            onClick={publish}
                            type="button"
                        >
                ...
                <div className="db-metrics">
                    ...
                    <DashboardMetric
                        ...
                        label="Anonymous"
                        value={analytics.anonymousResponses.toString()}
                    />
                    <DashboardMetric
                        icon={<UserCheck size={12} />}
                        label="Signed in"
                        value={analytics.authenticatedResponses.toString()}
                    />
                </div>
