...
export async function createPoll(req: Request, res: Response) {
    ...
    return res
        .status(201)
        .json({ poll: serializePoll(loaded.poll, loaded.questions) })
}
...
export async function publishPoll(req: Request, res: Response) {
    ...
    const [updated] = await db
        ...
        .where(
            and(eq(pollsTable.id, pollId), eq(pollsTable.createdBy, user.id))
        )
        .returning()
...
export async function reopenPoll(req: Request, res: Response) {
    ...
    if (
        loaded.poll.expiresAt &&
        loaded.poll.expiresAt.getTime() <= Date.now()
    ) {
...
export async function duplicatePoll(req: Request, res: Response) {
    ...
    const duplicated = await db.transaction(async (tx) => {
        ...
        if (!createdPoll)
            throw new HttpError(500, 'Poll could not be duplicated')
    ...
    if (!duplicatedLoaded)
        throw new HttpError(500, 'Duplicated poll could not be loaded')
...
export async function exportResponses(req: Request, res: Response) {
    ...
    const answers =
        responseIds.length > 0
            ? await db
                  ...
                  .where(
                      inArray(questionResponsesTable.responseId, responseIds)
                  )
            : []

    const optionLabelsByQuestion = new Map<string, Map<string, string>>()
    for (const question of loaded.questions) {
        const options = parseJson<{ id: string; label: string }[]>(
            question.options,
            []
        )
    ...
    for (const answer of answers) {
        ...
        const answerText =
            labels?.get(answer.selectedOptionId ?? '') ??
            answer.selectedOptionId ??
            ''
        const row =
            answersByResponse.get(answer.responseId) ??
    ...
    const rows = responses.map((response) => {
        const responseAnswers =
            answersByResponse.get(response.id) ?? new Map<string, string>()
        return [
            response.id,
            ...
            ...loaded.questions.map(
                (question) => responseAnswers.get(question.id) ?? ''
            ),
        ]
    })
    ...
    const questionsById = new Map(
        loaded.questions.map((question) => [
            question.id,
            {
                question: question.question,
                options: new Map(
                    parseJson<{ id: string; label: string }[]>(
                        question.options,
                        []
...
export async function getPublicPoll(req: Request, res: Response) {
    ...
    return res.json({
        ...
        state: {
            ...
            resultsVisible:
                loaded.poll.status === 'published' ||
                loaded.poll.showLiveResults,
        ...
        .transaction(async (tx) => {
            const [createdResponse] = await tx
                ...
                .values({
                    ...
                    respondentName: loaded.poll.isAnonymous
                        ? input.respondentName
                        : sessionUser?.name,
                    respondentEmail: loaded.poll.isAnonymous
            ...
            if (!createdResponse)
                throw new HttpError(500, 'Response could not be saved')

            const cleanAnswers = input.answers.filter((answer) =>
